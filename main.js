...
let gameState = {
  ...
  girlImageAlt:
    "/img/character.png",
  characterName: "Chica Anime", // Nombre por defecto del personaje
};
...
// Funciones para mostrar/ocultar modales centrados
...
const characterNameElement = document.getElementById("characterName"); // Elemento para el nombre

const dialogues = [
  {
    character: "Chica Anime", // Puedes variar el nombre si hay múltiples personajes
  ...
  },
  {
    ...
    options: [
      {
        text: "¡Claro que sí! Estás protegiendo a muchos.",
        affinityChange: 15,
      },
      {
        text: "Si tienes dudas, quizás deberías reconsiderarlo.",
        affinityChange: -10,
      },
...
function startDialogue() {
  ...
  characterNameElement.textContent =
    currentDialogue.character || gameState.characterName;
  ...
  currentDialogue.options.forEach((option) => {
    ...
    button.className = "dialogue-option-button"; // Nueva clase para opciones
    button.onclick = () => {
      ...
      showNotification(
        ...
        `Ella parece ${
          option.affinityChange > 0
            ...
            ? "algo molesta"
            : "neutral"
        } con tu respuesta. (Afinidad ${option.affinityChange > 0 ? "+" : ""}${
          option.affinityChange
        })`
      );
      hideTalkModal(); // Usar la función específica para ocultar el modal de diálogo
    };
    dialogueOptionsContainer.appendChild(button);
  });
  showTalkModal(); // Usar la función específica para mostrar el modal de diálogo
}
// No hay botón de cerrar explícito en el modal de diálogo RPG, se cierra con la opción.
...
document.querySelectorAll(".outfit-choice").forEach((button) => {
  button.addEventListener("click", (e) => {
    ...
    showNotification(
      "Equipamiento",
      `Atuendo cambiado a: ${e.target.textContent}`
    );
...
feedButton.addEventListener("click", () => {
  ...
  showNotification(
    "Alimentar",
    "Le diste algo de comer. ¡Parece contenta! (+5 Afinidad)"
  );
...
playMinigameButton.addEventListener("click", () => {
  showNotification(
    "Minijuego",
    "Esta función estará disponible pronto. ¡Imagina que jugaron un divertido juego!"
  );
  hideModal(interactModal);
});
closeInteractModalButton.addEventListener("click", () =>
  hideModal(interactModal)
);
...
const missionProgressContainer = document.getElementById(
...
function startMission() {
  ...
  missionProgressContainer.classList.remove("hidden"); // Asegúrate que el contenedor de la barra sea visible
  ...
  const interval = setInterval(() => {
    ...
    if (progress >= 100) {
      ...
      // missionProgressContainer.classList.add('hidden'); // Ocultar barra al completar
    }
  }, 300); // Duración de la misión simulada
}
closeMissionModalButton.addEventListener("click", () =>
...
// Desktop
document
  .getElementById("talkButtonDesktop")
  .addEventListener("click", startDialogue);
document
  .getElementById("equipButtonDesktop")
  .addEventListener("click", () => showModal(equipModal));
document
  .getElementById("interactButtonDesktop")
  .addEventListener("click", () => showModal(interactModal));
document
  .getElementById("missionButtonDesktop")
  .addEventListener("click", startMission);

// Mobile
document
  .getElementById("talkButtonMobile")
  .addEventListener("click", startDialogue);
document
  .getElementById("equipButtonMobile")
  .addEventListener("click", () => showModal(equipModal));
document
  .getElementById("interactButtonMobile")
  .addEventListener("click", () => showModal(interactModal));
document
  .getElementById("missionButtonMobile")
  .addEventListener("click", startMission);
